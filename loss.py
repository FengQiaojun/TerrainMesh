    ...
    def forward(self, meshes_pred, sem_2d_pred, meshes_gt, gt_depth, gt_semantic, return_img=False):
        ...
        for i, cur_meshes_pred in enumerate(meshes_pred):
            ...
            if (gt_semantic is not None and self.semantic_weight > 0):
                ...
                #criterion1 = FocalLoss(weight = torch.Tensor([0,1,3,3,1]).to(self.device), ignore_index=0, size_average=True)
                #criterion1 = FocalLoss(weight = torch.Tensor([0,1,3,1,3]).to(self.device), ignore_index=0, size_average=True)
                #criterion1 = FocalLoss(weight = torch.Tensor([0,1,2,3,3]).to(self.device), ignore_index=0, size_average=True)
                #criterion2 = DiceLoss(num_classes=5,device=self.device,eps=1e-7)
                #semantic_loss = criterion1(semantic_predict, gt_semantic) + criterion2(semantic_predict, gt_semantic)
                criterion2 = DiceLoss(num_classes=5,device=self.device,eps=1e-7)
                #criterion2 = JaccardLoss(num_classes=5,device=self.device,eps=1e-7)
                semantic_loss = criterion2(semantic_predict, gt_semantic)
...
class DiceLoss(nn.Module):
    def __init__(self, num_classes, device, eps=1e-7):
...
class JaccardLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.num_classes == 1:
            ...
            true_1_hot = torch.eye(self.num_classes)[targets.squeeze(1)]
            true_1_hot = true_1_hot.permute(0, 3, 1, 2).float()
            probas = F.softmax(inputs, dim=1)
        true_1_hot = true_1_hot.type(inputs.type())
        dims = (0,) + tuple(range(2, targets.ndimension()))
