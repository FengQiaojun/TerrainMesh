...
while cur_epochs < total_epochs:
    ...
    for (images, labels) in train_loader:
        ...
        img_predict_2d = outputs.detach().cpu().numpy()[0,:,:]
        img_predict_2d = np.argmax(img_predict_2d,axis=0)
        ...
        mesh_img, mesh_depth = render_mesh_vertex_texture(verts=vertices[0,:],faces=faces[0,:],feats=vert_align_feats,image_size=64,device=device)
        print(mesh_img.shape)
        # step 5: feed the rendered image back to the classifier 
        featuremap = OrderedDict()
        featuremap["out"] = mesh_img.permute(0,3,1,2)
        img_predict = model.classify_features(featuremap, input_shape=[512,512])
        ...
        Image.fromarray(convert_class_to_rgb(img_predict)).save('%d_mesh1024_feat.png'%cur_itrs)
