...
class TerrainDataset(Dataset):
    def __init__(
        ...
        if meshing == "regular":
            ...
            raise ValueError("Only accept meshing = [\"regular\",\"superpixel\",\"sfm\"]")
        
        if samples not in [2048,1024,512]:
            raise ValueError("Only accept samples = [512,1024,2048]")
        if split not in ["train","val","test"]:
            raise ValueError("Only accept split = [\"train\",\"val\",\"test\"]")
        ...
        for seq in seq_idx_list:
            for target in sorted(os.listdir(os.path.join(self.root,seq,"Images"))):
                target_idx = target[:-4]
                if noise:
                    self.rgb_img_ids.append(os.path.join(self.root,seq,"Images",target))
                    self.sparse_depth_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples),target))
                    self.mesh_render_depth_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples),target_idx+init_mesh_ext+".png"))
                    self.depth_edt_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples),target_idx+"_edt.pt"))
                    self.gt_depth_ids.append(os.path.join(self.root,seq,"Depths",target))
                    self.init_mesh_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples),target_idx+init_mesh_ext+".obj"))
                    self.gt_mesh_ids.append(os.path.join(self.root,seq,"Meshes",target_idx+".pt"))
                    self.data_ids.append(os.path.join(seq,target_idx))
                else:
                    self.rgb_img_ids.append(os.path.join(self.root,seq,"Images",target))
                    self.sparse_depth_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples)+"_gt",target))
                    self.mesh_render_depth_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples)+"_gt",target_idx+init_mesh_ext+".png"))
                    self.depth_edt_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples)+"_gt",target_idx+"_edt.pt"))
                    self.gt_depth_ids.append(os.path.join(self.root,seq,"Depths",target))
                    self.init_mesh_ids.append(os.path.join(self.root,seq,"Pcds_"+str(samples)+"_gt",target_idx+init_mesh_ext+".obj"))
                    self.gt_mesh_ids.append(os.path.join(self.root,seq,"Meshes",target_idx+".pt"))
                    self.data_ids.append(os.path.join(seq,target_idx))
    ...
    def __getitem__(self, idx):
        ...
        sparse_depth = np.asfarray(imread(sparse_depth_path)/64, dtype=np.float32)/1000
        sparse_depth = T.ToTensor()(sparse_depth)
        mesh_render_depth = np.asfarray(imread(mesh_render_depth_path)/64, dtype=np.float32)/1000
        mesh_render_depth = T.ToTensor()(mesh_render_depth)
        depth_edt = torch.clamp(torch.load(depth_edt_path).float()/20,min=0,max=2)
    ...
    def collate_fn(batch):
        rgb_img, sparse_depth, mesh_render_depth, depth_edt, gt_depth, init_mesh_v, init_mesh_f, gt_mesh, data_id = zip(*batch)
        ...
        if init_mesh_v[0] is not None and init_mesh_f[0] is not None:
            init_mesh = Meshes(verts=list(init_mesh_v), faces=list(init_mesh_f))
        else:
            init_mesh = None
...
def load_data_by_index(cfg,
        ...
        if noise:
            sparse_depth_path = os.path.join(root,seq_idx,"Pcds_"+str(samples),img_idx+".png")
            mesh_render_depth_path = os.path.join(root,seq_idx,"Pcds_"+str(samples),img_idx+init_mesh_ext+".png")
            depth_edt_path = os.path.join(root,seq_idx,"Pcds_"+str(samples),img_idx+"_edt.pt")
            init_mesh = os.path.join(root,seq_idx,"Pcds_"+str(samples),img_idx+init_mesh_ext+".obj")
        else:
            sparse_depth_path = os.path.join(root,seq_idx,"Pcds_"+str(samples)+"_gt",img_idx+".png")
            mesh_render_depth_path = os.path.join(root,seq_idx,"Pcds_"+str(samples)+"_gt",img_idx+init_mesh_ext+".png")
            depth_edt_path = os.path.join(root,seq_idx,"Pcds_"+str(samples)+"_gt",img_idx+"_edt.pt")
            init_mesh = os.path.join(root,seq_idx,"Pcds_"+str(samples)+"_gt",img_idx+init_mesh_ext+".obj")
