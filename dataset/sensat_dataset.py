...
class SensatSemanticDataset(Dataset):
    def __init__(self,data_dir,split=None,meshing=None,samples=None,depth_scale=None,normalize_images=True,):
        ...
        for seq in seq_idx_list:
            for target in sorted(os.listdir(os.path.join(data_dir,seq,"Images"))):
                ...
                self.sparse_depth_ids.append(os.path.join(data_dir,seq,"Pcd_"+str(samples),target))
                self.depth_edt_ids.append(os.path.join(data_dir,seq,"Pcd_"+str(samples),target))
                self.init_mesh_ids.append(os.path.join(data_dir,seq,"Pcd_"+str(samples),target_idx+"_"+meshing+".obj"))
                self.init_mesh_render_depth_ids.append(os.path.join(data_dir,seq,"Pcd_"+str(samples),target_idx+"_"+meshing+".png"))
                self.gt_depth_ids.append(os.path.join(data_dir,seq,"Depths",target))
                self.gt_mesh_pcd_ids.append(os.path.join(data_dir,seq,"Meshes",target_idx+".pt"))
                self.sem_img_ids.append(os.path.join(data_dir,seq,"Semantics_5",target))
    ...
    def __getitem__(self, idx):
        ...
        sparse_depth = np.asfarray(imread(sparse_depth_path)/self.depth_scale, dtype=np.float32)/depth_input_scale
        ...
        init_mesh_v, init_mesh_f, _ = load_obj(init_mesh_path,load_textures=False)
        init_mesh_f = init_mesh_f.verts_idx
        init_mesh_render_depth = np.asfarray(imread(init_mesh_render_depth_path)/self.depth_scale, dtype=np.float32)/depth_input_scale
        init_mesh_render_depth = transforms.ToTensor()(init_mesh_render_depth)
        gt_depth = np.asfarray(imread(gt_depth_path)/self.depth_scale, dtype=np.float32)
    ...
    def collate_fn(batch):
        rgb_img, sparse_depth, depth_edt, init_mesh_v, init_mesh_f, init_mesh_render_depth, gt_depth, gt_mesh_pcd, sem_img = zip(*batch)
